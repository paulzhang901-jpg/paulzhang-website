...
export async function HomePage({locale}: {locale: Locale}) {
  ...
  return <>
    ...
    <section data-home-section="about" className={`${section} bg-muted/60`}><Container><div className="grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_18rem]"><div className="max-w-3xl"><h2 className="font-serif text-3xl sm:text-4xl">{c.about.title}</h2><p className="mt-5 text-lg leading-8 text-muted-foreground">{c.about.body}</p><TrackedLink className={`${linkButton} mt-7 border bg-surface`} href={localizedPath("about", locale)} event={event} payload={{locale, source: "about", destination_type: "about"}}>{c.about.cta}</TrackedLink></div><div className="mx-auto w-full max-w-64 rounded-[2rem] border bg-surface p-3 shadow-[var(--shadow-soft)] lg:max-w-72"><Image src="/brand/paul-zhang/paul-zhang-seal-A-primary.png" alt={c.about.sealAlt} width={431} height={392} sizes="(max-width: 1024px) 16rem, 18rem" className="h-auto w-full rounded-[1.4rem]" /></div></div></Container></section>
