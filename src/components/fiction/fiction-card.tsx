...
export function FictionCard({work, locale, priority = false}: {work: FictionCardRecord; locale: "zh-CN" | "en-US"; priority?: boolean}) {
  return <article className="group flex h-full flex-col overflow-hidden rounded-[1.1rem] border bg-surface shadow-[var(--shadow-soft)]">
    <Link href={fictionPath(work.slug, locale)} className="relative block aspect-[3/4] overflow-hidden bg-muted">
    ...
    <div className="flex flex-1 flex-col p-5 sm:p-6">
      <h3 className="text-balance font-serif text-2xl leading-[1.35]"><Link href={fictionPath(work.slug, locale)} className="hover:text-primary">《{work.canonicalTitle}》</Link></h3>
