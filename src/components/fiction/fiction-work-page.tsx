...
export function FictionWorkPage({work, locale}: {work: FictionEditorialRecord; locale: "zh-CN" | "en-US"}) {
  ...
  return <>
    ...
    <section className="border-b bg-[#18251f] text-[#f7f0df]">
      <Container className="grid gap-10 py-12 md:grid-cols-[minmax(16rem,0.62fr)_minmax(0,1fr)] md:items-center md:gap-12 lg:gap-16 lg:py-20">
        <div className="relative mx-auto aspect-[3/4] w-full max-w-[25rem] overflow-hidden rounded-[1.1rem] bg-black/20 shadow-2xl"><Image src={getFictionCoverPath(work)} alt={`《${work.canonicalTitle}》封面`} fill priority sizes="(min-width: 1024px) 38vw, (min-width: 768px) 40vw, 86vw" className="object-contain" /></div>
        <div className="min-w-0"><Link href={fictionPath(undefined, locale)} className="inline-flex min-h-11 items-center text-sm text-[#d4b27f] hover:underline">← {labels.back}</Link><p className="mt-6 text-sm uppercase tracking-[0.2em] text-[#d4b27f]">Mu Changke Fiction</p><h1 className="mt-3 max-w-[15ch] text-balance font-serif text-4xl leading-[1.15] sm:text-5xl lg:text-6xl">《{work.canonicalTitle}》</h1><p className="mt-4 text-[#d9d3c7]">{labels.author}</p>{work.themes.length ? <div className="mt-6"><h2 className="sr-only">{labels.themes}</h2><ul className="flex flex-wrap gap-2">{work.themes.map((theme) => <li key={theme} className="rounded-full border border-white/20 px-3 py-1 text-sm text-[#e6ddcf]">{theme}</li>)}</ul></div> : null}{work.editorialHook ? <p className="mt-7 max-w-[36em] font-serif text-xl leading-[1.75] text-[#eee4d2] sm:text-2xl"><RichText text={work.editorialHook} /></p> : null}<a href="#official-reading-guidance" className="mt-7 inline-flex min-h-12 items-center rounded-full border border-[#d4b27f]/60 px-5 font-medium text-[#f7f0df] transition-colors hover:bg-white/10">{labels.cta}<span aria-hidden="true" className="ml-2">↓</span></a></div>
      </Container>
    </section>
    <ReadingContainer className="space-y-12 py-[var(--space-section)]">
      {sections}
      <aside id="official-reading-guidance" aria-labelledby="fiction-cta-heading" className="scroll-mt-24 rounded-[1.1rem] border border-primary/25 bg-muted p-7 sm:p-9"><p className="text-sm font-semibold uppercase tracking-[0.18em] text-primary">Search guidance</p><h2 id="fiction-cta-heading" className="mt-2 font-serif text-3xl">{labels.cta}</h2><div className="mt-5 space-y-4 whitespace-pre-line text-[1.0625rem] leading-8"><RichText text={work.officialReadingCTA} /></div><p className="mt-6 border-t pt-5 text-sm leading-6 text-muted-foreground">{labels.boundary}</p></aside>
    </ReadingContainer>
  </>;
}
