...
export function metadataForWork(work: ContentWork, representation: PublicWorkRepresentation, repository: ContentWorkRepository): Metadata {
  ...
  if (zh.available) languages["zh-CN"] = new URL(workPath(zh.representation.slug, "zh-CN"), siteUrl).toString();
  if (en.available) languages["en-US"] = new URL(workPath(en.representation.slug, "en-US"), siteUrl).toString();
...
export function metadataForUnit(work: ContentWork, workRepresentation: PublicWorkRepresentation, unit: ContentUnit, repository: ContentWorkRepository): Metadata {
  ...
  const languages: Record<string, string> = {};
  if (zh.available) languages["zh-CN"] = new URL(unitPath(zh.workRepresentation.slug, zh.unit.slug, "zh-CN"), siteUrl).toString();
  if (en.available) languages["en-US"] = new URL(unitPath(en.workRepresentation.slug, en.unit.slug, "en-US"), siteUrl).toString();
