...
export async function generateStaticParams() { return (await getUnitStaticParams("zh-CN", await getContentWorkRepository())).map(({workSlug, unitSlug}) => ({slug: workSlug, unitSlug})); }
export async function generateMetadata({params}: Props) {
  const repository = await getContentWorkRepository();
  const {slug, unitSlug} = await params;
  const result = resolveUnitRoute(slug, unitSlug, "zh-CN", repository);
