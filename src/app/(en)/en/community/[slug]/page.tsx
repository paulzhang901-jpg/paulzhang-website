...
export async function generateMetadata({params}:Props){const slug=(await params).slug;return isCommunitySection(slug)?metadataForCollectionRoute("community",slug,"en-US"):{};}
export default async function Page({params}:Props){const slug=(await params).slug;if(!isCommunitySection(slug))notFound();return <CommunitySectionPage locale="en-US" section={slug}/>}
