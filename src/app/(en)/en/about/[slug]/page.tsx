...
export async function generateMetadata({params}:Props){const slug=(await params).slug;return isAboutSection(slug)?metadataForCollectionRoute("about",slug,"en-US"):{};}
export default async function Page({params}:Props){const slug=(await params).slug;if(!isAboutSection(slug)||slug==="contact"||slug==="support")notFound();return <AboutSectionPage locale="en-US" section={slug}/>}
