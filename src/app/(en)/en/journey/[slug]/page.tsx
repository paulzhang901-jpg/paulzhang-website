...
export async function generateMetadata({params}:Props){const slug=(await params).slug;return isLightJourneySection(slug)?metadataForCollectionRoute("journey",slug,"en-US"):{};}
export default async function Page({params}:Props){const slug=(await params).slug;if(!isLightJourneySection(slug))notFound();return <LightJourneySectionPage locale="en-US" section={slug}/>}
