...
test("Community church remains permanent section 01 and preserves sections 02-07",()=>{
...
test("church profile reuses verified canonical church links and current owner-provided location",()=>{
...
test("church page contains approved bilingual facts and excludes stale or unverified specifics",()=>{
  const page=fs.readFileSync("src/components/content/church-introduction-page.tsx","utf8");
  for(const value of ["First Chinese Free Methodist Church of Indianapolis","我们是谁","我们的宗派传统","我们的属灵归属","我们现在在哪里","寻找一个可以扎根的家","在线认识我们","Who We Are","Our Free Methodist Heritage","Our Conference","Where We Gather Today","Looking for a Place to Call Home","Connect With Our Church Online"]) assert.match(page,new RegExp(value));
...
test("church metadata has bilingual canonical and hreflang while preserving contract",()=>{
  const zh=metadataForCommunitySection("church","zh-CN"); const en=metadataForCommunitySection("church","en-US");
  assert.equal(zh.title,"印城第一华人循理会"); assert.equal(en.title,"First Chinese Free Methodist Church of Indianapolis");
...
test("canonical church social data remains unmodified",()=>{
